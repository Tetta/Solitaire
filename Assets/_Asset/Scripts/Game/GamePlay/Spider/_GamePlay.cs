	...
	public class _GamePlay
	{
		...
		{  
			...
			case Enums.ModeGame.Hard:
				...
				// TODO: Loop 2 times to get the cards.
				for (int i = 0; i < 2; i++) {
		...
		IEnumerator < float > DrawingGameHandle()
		{
			...
			// TODO: Loop 40 times to draw the cards.
			for (int i = 0; i < 40; i++)
			{
                ...
                //Debug.Log("cardCache5 -- " + cardCache.TargetPosition);
			...
			for (int i = 40; i <= lenght; i += PlayingZone.Instance.GetLenghtZone()) {
				...
				// TODO: Get the dafult transform from zone.
				Transform parentTransform = HintZone.Instance.GetTransformCards (idTransform);

				// TODO: Get the position of card from zone.
				position = Helper.GetPositionInTheHintZone (idTransform, Enums.Direction.Left); 

				for (int j = 0; j < PlayingZone.Instance.GetLenghtZone(); j++) {

					// TODO: Set value of card.
					cardCache = cardsGet [i + j];
					...
					// TODO: Moving the card to new position.
					cardCache.Moving ( cardCache.TargetPosition , parentTransform );
                    //Debug.Log("cardCache7 -- " + cardCache.TargetPosition);
		...
		#region Condition Spider Solitaire
