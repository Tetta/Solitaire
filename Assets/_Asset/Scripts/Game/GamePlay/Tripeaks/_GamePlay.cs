	...
	public class _GamePlay{
		...
		{  
			...
			for (int i = 0; i < count; i++) {
				...
				paramCache.transform.localScale = Contains.Vector3One;

				// TODO: Set the default of position.
				paramCache.transform.localPosition = Contains.Vector3Zero;
		...
		IEnumerator < float > DrawingGameHandle()
		{
			...
			// TODO: Loop 70 times to draw the cards.
			for (int i = 0; i < 56; i++) {
                ...
                //Debug.Log("cardCache4 -- " + cardCache.TargetPosition);
			...
			for (int i = 56; i <= lenght; i++) {
                ...
                //Debug.Log("cardCache6 -- " + cardCache.TargetPosition);
		...
		#region Condition Tripeak Solitaire
		...
		{
			if ( PlayingZone.Instance.IsEmptyAllCards() )
			{
